...
void sleep_timer_handler(void *m_notused){
    ...
    if(voltage < 3.2)  
    {
        ...
        led_task();
        //int predictor = predict_when(3.5);
        ...
        if(a<1){
            ...
            sensing_gpio_task();
            a = 1;
            ERROR_CHECK(app_timer_start(sleep_timer, HAL_SECS_TO_RTC_TICKS(10), NULL));
            //nrf_drv_gpiote_out_clear(PIN_OUT_LED);
        }else{
            ...
            send_gpio_task();
            a = 0;
            ERROR_CHECK(app_timer_start(sleep_timer, HAL_SECS_TO_RTC_TICKS(120), NULL));
            //nrf_drv_gpiote_out_clear(PIN_OUT_LED2);
...
void schedule_Task(void *n){
    ...
    ERROR_CHECK(app_timer_start(sleep_timer, HAL_MS_TO_RTC_TICKS(5), NULL));
