...
/* Flash parts */
/* Tasks part */
uint32_t pg_size_tasks;
uint32_t pg_num_tasks;
struct Task *pointer_tasks;
...
void saadc_init(void)
{
    ...
    nrf_saadc_channel_config_t channel_config = NRFX_SAADC_DEFAULT_CHANNEL_CONFIG_SE(PIN_CAP_READ);
...
void gpio_init_cap(void)
{
    __LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "----- gpio_init -----\n");   
...
uint8_t if_energy(uint16_t v_th)
{
    nrf_drv_gpiote_out_set(PIN_OUT);
    //__LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "UP \n");
    ret_code_t err_code = app_timer_start(gpio_timer_id,HAL_MS_TO_RTC_TICKS(GPIO_ENABLE_TIME),NULL);
    APP_ERROR_CHECK(err_code);
    nrf_saadc_value_t cap_val = get_cap_val();
    //__LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "Cap value %d \n",cap_val);
    if (v_th <= cap_val)
    {
        return 1;
    }
...
void led2_timeout_handler(){
    
    ret_code_t err_code;
    err_code = app_timer_stop(led2_timer_id);
    APP_ERROR_CHECK(err_code);
    nrf_drv_gpiote_out_toggle(PIN_OUT_LED2);
}

void gpio_timeout_handler()
{
    ret_code_t err_code;
    err_code = app_timer_stop(gpio_timer_id);
    APP_ERROR_CHECK(err_code);
    nrf_drv_gpiote_out_toggle(PIN_OUT);
    //__LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "Down \n");
}

void sensing_timeout_handler()
{
    ret_code_t err_code;
    err_code = app_timer_stop(sense_timer_id);
    APP_ERROR_CHECK(err_code);
    nrf_drv_gpiote_out_toggle(PIN_OUT_SENSE);
}

void send_timeout_handler()
{
    ret_code_t err_code;
    err_code = app_timer_stop(send_timer_id);
    APP_ERROR_CHECK(err_code);
    nrf_drv_gpiote_out_toggle(PIN_OUT_SEND);
}
...
void f_transmit(uint8_t *data)
{
    ...
    /* Function defined in simple_message_client.c file -> see for more details */
...
void f_temp()
{
    ...
    /* More details in nrf_soc.h file -> right click and go to Go To Definition */
...
void f_store()
{
    /* More information about mesh config functions related to flash storage can be found in mesh_config.c file */
...
void led2_task(){
    ...
    nrf_drv_gpiote_out_set(PIN_OUT_LED2);
    ret_code_t err_code = app_timer_start(led2_timer_id,HAL_MS_TO_RTC_TICKS(LED2_ENABLE_TIME),NULL);
    APP_ERROR_CHECK(err_code);
} 

void sensing_gpio_task(){

    nrf_drv_gpiote_out_set(PIN_OUT_SENSE);
    ret_code_t err_code = app_timer_start(sense_timer_id, HAL_MS_TO_RTC_TICKS(SENSE_ENABLE_TIME), NULL);
    APP_ERROR_CHECK(err_code);
}

void send_gpio_task(){

    nrf_drv_gpiote_out_set(PIN_OUT_SEND);
    ret_code_t err_code = app_timer_start(send_timer_id, HAL_MS_TO_RTC_TICKS(SEND_ENABLE_TIME), NULL);
    APP_ERROR_CHECK(err_code);
}
...
float read_Voltage(){
    ...
    //float read_voltage = (adc_val * 3.6 / 1024) * 3.1;
    float read_voltage = (adc_val * 3.6 / 1024) * 2.0;
...
float cap_task(){
    ...
    float cap_val = read_Voltage();
    
    //char sdata[8];
    //sprintf(sdata, "Cap %d", cap_val);
    //__LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "Cap Vol: %d\n", cap_val);
    return cap_val;
}
...
void initialize(void)
{
    ...
    err_code = nrf_drv_gpiote_out_init(PIN_OUT_LED2, &out_config);
    err_code = nrf_drv_gpiote_out_init(PIN_OUT_SENSE, &out_config);
    err_code = nrf_drv_gpiote_out_init(PIN_OUT_SEND, &out_config);
    APP_ERROR_CHECK(err_code);
...
void start(void)
{
    ...
    ERROR_CHECK(app_timer_create(&led2_timer_id,APP_TIMER_MODE_SINGLE_SHOT,led2_timeout_handler));
    ERROR_CHECK(app_timer_create(&gpio_timer_id,APP_TIMER_MODE_SINGLE_SHOT,gpio_timeout_handler));
    ERROR_CHECK(app_timer_create(&sense_timer_id,APP_TIMER_MODE_SINGLE_SHOT,sensing_timeout_handler));
    ERROR_CHECK(app_timer_create(&send_timer_id,APP_TIMER_MODE_SINGLE_SHOT,send_timeout_handler));
