...
void sleep_timer_handler(void *m_notused){
    ...
    handle_new_data(voltage);

    store_voltage_in_flash

    if(voltage < req_voltage) -> required for sensing 
    {
        ERROR_CHECK(app_timer_start(sleep_timer, HAL_MS_TO_RTC_TICKS(sleep_time), NULL)); 
    }else{
        ...
        unsigned int next_time = 3000;

        //__LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "Sleeping for %d ms\n", next_time);

        //

        ERROR_CHECK(app_timer_start(task_timer, HAL_MS_TO_RTC_TICKS(next_time), NULL));      
    }
...
void schedule_Task(void *n){
   ...
   //input parameter which is the required voltage threshold of that specific task
    ...
    int predictor = predict_when(3.2);

    sleep_time = (sleep_time*predictor);

    __LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "Predicted value is: %d\n!", predictor);

    ERROR_CHECK(app_timer_start(sleep_timer, HAL_MS_TO_RTC_TICKS(1), NULL));
