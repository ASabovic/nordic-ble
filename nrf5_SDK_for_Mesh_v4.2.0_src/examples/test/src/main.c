...
void getfirstTask(){
    ...
    application_dofile("C:/Users/IDLab guest/Documents/GitHub/BLUESS_BLE/nrf5_SDK_for_Mesh_v4.2.0_src/examples/test/src/tasks.json", "C:/Users/IDLab guest/Documents/GitHub/BLUESS_BLE/nrf5_SDK_for_Mesh_v4.2.0_src/examples/test/src/tasksflow.json");
...
//                    task_instances[i].start_time = tasks[selected_task->task_id].execution_time + tasks[selected_task->task_id].constraint_value[m];
                    task_instances[i].start_time = tasks[selected_task->task_id].execution_time + 1000;
                    task_instances[i].task_id = tasks[selected_task->task_id].child_id[m];
                    instances_id = ENTRY_TSK_ID;
                    instances_id.record += i;
                    mesh_config_entry_set(instances_id, &task_instances[i]);
...
void start_app(void *s){

    start_voltage = read_Voltage();

    if(start_voltage < 260000){
    
       ERROR_CHECK(app_timer_start(start_timer, HAL_MS_TO_RTC_TICKS(5000), NULL)); 
    }else{
...
void start_timers(){
    ...
    ERROR_CHECK(app_timer_create(&start_timer, APP_TIMER_MODE_SINGLE_SHOT, start_app));
}
...
int main(void){
    ...
    if(mesh_stack_is_device_provisioned()){
        ...
        ERROR_CHECK(app_timer_start(start_timer, HAL_MS_TO_RTC_TICKS(1), NULL));
    }else{
