...
/* Define a initial voltage timer */
...
double read_Voltage(){
    ...
    double read_voltage = adc_val * 3.6 / 1024;
...
void getfirstTask(){
    ...
    /* Read and parse a JSON file/s into a struct/s */
    application_dofile("C:/Users/IDLab guest/Documents/GitHub/BLUESS_BLE/nrf5_SDK_for_Mesh_v4.2.0_src/examples/predictor/src/tasks.json", "C:/Users/IDLab guest/Documents/GitHub/BLUESS_BLE/nrf5_SDK_for_Mesh_v4.2.0_src/examples/predictor/src/tasksflow.json");
...
void sleep_timer_handler(void *m_notused){
    ...
    //// input of this function will be previously measured voltage - 2.4V
    handle_new_aperiodic_synchronous_data(voltage, in_step);

    //store_voltage_in_flash

    if(voltage < 3.15) //-> required for sensing 
    {
        int predictor = predict_when(3.15);

        sleep_time = (sleep_time*predictor);
        ERROR_CHECK(app_timer_start(sleep_timer, HAL_SECS_TO_RTC_TICKS(sleep_time), NULL)); 
    }else{
        ...
        //__LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "Required voltage threshold is satisfied! \n");


        //send_interrupt_to_arduino_to_sense


        led_task();

        __LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "Done with LED task!\n");

        //int predictor = predict_when(1.5);

        //sleep_time = (sleep_time*predictor);

        unsigned int next_time = fmax(get_time(), 5);

        //__LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "Sleeping for %d ms\n", next_time);

        //__LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "Sleeping for %d ms\n", next_time);

        //

        ERROR_CHECK(app_timer_start(task_timer, HAL_MS_TO_RTC_TICKS(5), NULL));      
    }
...
void schedule_Task(void *n){
    ...
    //selected_task = removeTask();
    //__LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "Selected task: %d\n", tasks[selected_task->task_id].task_name);

    //after I choose the highest priority task to be executed the predict_when function should be called with the 
   //input parameter which is the required voltage threshold of that specific task
    ...
    //__LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "Predicted value is: %d\n!", predictor);
...
void collect_data(void *k){
    ...
    __LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "Initial voltage is: %d\n!", in_voltage);

    if(in_voltage<3.5){
        __LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "It's okay!\n"); 
    }else{
        __LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "It's not okay!\n");
    }
    ...
    if(in_step<11){
        ...
        in_step = in_step+1;
        __LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "Initial step is: %d\n", in_step);
        ERROR_CHECK(app_timer_start(in_timer, HAL_SECS_TO_RTC_TICKS(sleep_time), NULL));
    }else{
    
        __LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "I am done!\n");
        schedule_Task(NULL);
