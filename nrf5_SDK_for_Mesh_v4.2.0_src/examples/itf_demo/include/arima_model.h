...
static fptype Y[BUF_LEN]; //!< Buffer to store observed time series data
static fptype E[_Q];       //!< Buffer to store residual (observed-predicted) terms
...
static fptype Yt_predicted = 0;

//! Time_series data length tracker
static uint32_t ts_length = 0;
...
void handle_new_data_core(fptype b_Yt, int *y_latest_val_at_index_ptr, int *e_latest_val_at_index_ptr, fptype y[], fptype e[], fptype yt_predicted) {
  ...
  if (y_latest_val_at_index == BUF_LEN)
    y_latest_val_at_index = 0;
  y[y_latest_val_at_index] = b_Yt;
#endif

#if _Q != 0
  e_latest_val_at_index++;
  if (e_latest_val_at_index == _Q)
    e_latest_val_at_index = 0;
...
void handle_new_data(fptype B_Yt /**< [in] The latest observed value. */) {
  ts_length++;
  handle_new_data_core(B_Yt, &Y_latest_val_at_index, &E_latest_val_at_index, Y, E, Yt_predicted);
}

fptype predict_next_one();//Forward declaration
void handle_new_aperiodic_synchronous_data(fptype B_Yt /**< [in] The latest observed value. */, int step) {
  ts_length++; // Assumed non-volatile storage
  // printf("ARIMA data handle: step = %d, ts_length=%d\n",step,ts_length);
  while(step != ts_length){
      ts_length++;
      //Easy but could be inefficient
      fptype temp_y = predict_next_one();
      handle_new_data_core(temp_y, &Y_latest_val_at_index, &E_latest_val_at_index, Y, E, Yt_predicted);
  }
  handle_new_data_core(B_Yt, &Y_latest_val_at_index, &E_latest_val_at_index, Y, E, Yt_predicted);  

  //Debug purposes only
  // int start_idx = Y_latest_val_at_index;
...
fptype predict_th1(int y_latest_val_at_index, int e_latest_val_at_index, fptype y[], fptype e[]) {
  // Assume time horizon = 1, just one step ahead,

  int idx = y_latest_val_at_index;

  fptype AR = 0; // Container for the AR part of the algorithm
...
//Macro to avoid unecessary array G loading + compute when D=0
    ...
    fptype G[BUF_LEN];
    // fptype G[BUF_LEN] = all the values loaded in respective index
...
fptype predict_vth(int th) {
  // if(virt_ts_length == 0) {

  static fptype Y_virt[BUF_LEN];
  for (int x = 0; x < BUF_LEN; x++)
    Y_virt[x] = Y[x];

  static fptype E_virt[_Q];
  for (int x = 0; x < _Q; x++)
    E_virt[x] = E[x];

  int Y_latest_val_at_index_virt = Y_latest_val_at_index;
  int E_latest_val_at_index_virt = E_latest_val_at_index;
  fptype Yt_predicted_virt = Yt_predicted;
  // static fptype [Yt];
  // }
  ...
  for (int y = 0; y < th; y++) {
    ...
    Yt_predicted_virt = predict_th1(Y_latest_val_at_index_virt, E_latest_val_at_index_virt, Y_virt,
        E_virt);
    handle_new_data_core(Yt_predicted_virt, &Y_latest_val_at_index_virt, &E_latest_val_at_index_virt, Y_virt, E_virt, Yt_predicted_virt);
  }
...
int predict_when(fptype joule_required) {
  // if(virt_ts_length == 0) {
  ...
  fptype total_energy = 0;
  for (int y = 0; y < MAX_TIME_HORIZON_STEPS; y++) {
    // virt_ts_length++;
    Yt_predicted_virt = predict_th1(Y_latest_val_at_index_virt, E_latest_val_at_index_virt, Y_virt,
        E_virt);

    if (Yt_predicted_virt < MIN_HARVESTING_POWER_W)
      Yt_predicted_virt = MIN_HARVESTING_POWER_W;
    else if (Yt_predicted_virt > MAX_HARVESTING_POWER_W)
      Yt_predicted_virt = MAX_HARVESTING_POWER_W;

    handle_new_data_core(Yt_predicted_virt, &Y_latest_val_at_index_virt, &E_latest_val_at_index_virt, Y_virt, E_virt, Yt_predicted_virt);

    total_energy += Yt_predicted_virt * SAMPLING_TIME_PERIOD_S;

    if (total_energy >= joule_required)
      return (y + 1);
  }
