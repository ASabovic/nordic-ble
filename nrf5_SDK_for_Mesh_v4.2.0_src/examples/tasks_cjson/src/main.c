...
char *create_taskJSON(){
    ...
    char *task_out;
    cJSON *task_json, *tasks, *task, *energyConsumption, *executionTime, *inputs, *output;

    task_json = cJSON_CreateObject();
    tasks = cJSON_CreateArray();

    cJSON_AddItemToObject(task_json, "Tasks", tasks);
    ...
    //Add second task to tasks array - Saving temperature values in flash 
    cJSON_AddItemToArray(tasks, task = cJSON_CreateObject());
    ...
    task_out = cJSON_Print(task_json);
...
void parse_taskJson(char * json_file){
    ...
    *priority, *inputs, *input_name, *input_type, *output, *output_name, *output_type;
    ...
    cJSON_ArrayForEach(task, tasks) 
    {
        ...
        if ((functionName->valuestring = "f_transmit") && (value_time->valuedouble == 10) && (unit_energy->valuestring = "mA"))
        {
...
int main(){
    ...
    char * task_out = create_taskJSON();
    printf("%s\n", task_out);
    parse_taskJson(task_out);
    free(task_out);
    return 0;    
}