...
void getfirstTask(){
    ...
    application_dofile("C:/Users/IDLab guest/Documents/GitHub/BLUESS_BLE/nrf5_SDK_for_Mesh_v4.2.0_src/examples/energy_aware_led/src/tasks.json", "C:/Users/IDLab guest/Documents/GitHub/BLUESS_BLE/nrf5_SDK_for_Mesh_v4.2.0_src/examples/energy_aware_led/src/tasksflow.json");
...
void sleep_timer_handler(void *m_notused){
    ...
    /* Check if the required voltage threshold of the selected (highest priority) task is satisfied */
    if(voltage < tasks[selected_task->task_id].required_voltage)
    {
        /* Sleep and check the voltage again after predefined period of time */
        ERROR_CHECK(app_timer_start(sleep_timer, HAL_MS_TO_RTC_TICKS(SLEEP_TIMEOUT_MS), NULL)); 
    }else{

        /* Execute the selected (highest priority) task*/
        execute();

        /* Add all dependent (child) tasks of the selected one in the task instances list and store it in the flash */
        addTask();


        /* Calculate the time when the next task can be selected */
        unsigned int next_time = fmax(get_time(), 5);

        __LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "Sleeping for %d ms\n", next_time);

        /* Sleep for next_time period before the schedule_Task function is called */
